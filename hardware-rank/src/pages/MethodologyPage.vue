    ...
    <article class="max-w-3xl space-y-8 min-w-0">
      ...
      <section v-for="s in METHOD" :id="s.id" :key="s.id" class="card p-5 space-y-3 scroll-mt-20">
        ...
        <template v-for="(b, i) in s.blocks" :key="i">
          ...
          <div v-else-if="b.type === 'table'" class="overflow-x-auto -mx-1">
            <table class="w-full text-sm">
              <thead class="text-xs text-muted"><tr><th v-for="h in b.head" :key="p(h)" class="text-left px-2 py-2 whitespace-nowrap">{{ p(h) }}</th></tr></thead>
              <tbody><tr v-for="(row, ri) in b.rows" :key="ri" class="border-t border-line/60 align-top"><td v-for="(c, ci) in row" :key="ci" class="px-2 py-2" :class="ci === 0 ? 'font-semibold whitespace-nowrap' : ''">{{ p(c) }}</td></tr></tbody>
            </table>
          </div>
